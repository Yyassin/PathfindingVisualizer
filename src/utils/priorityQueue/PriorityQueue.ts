...
class PriorityQueue<T> implements PriorityQueueInterface<T> {
     ...
     */
    private hasParent = (idx: number): boolean => Math.floor((idx - 1) / 2) >= 0;
    private hasLeft = (idx: number): boolean => this.left(idx) < this.heap.length;
    private hasRight = (idx: number): boolean => this.right(idx) < this.heap.length;
    ...
    private getMinChild(idx: number): number {
        ...
        if ( this.heap[this.right(idx)].getKey()
            < this.heap[this.left(idx)].getKey() ) { 
                return this.right(idx); 
        }
        if (this.heap[this.right(idx)].getKey()
            === this.heap[this.left(idx)].getKey()) {
                if ( this.heap[this.right(idx)].getSecondary()
    ...
    private percDown(curIdx: number): void {
        while (this.hasLeft(curIdx)) {
            ...
            if ( this.heap[minChildIdx].getKey() 
                < this.heap[curIdx].getKey() ) {
                    this.swap(curIdx, minChildIdx);
            }
            else if ( this.heap[minChildIdx].getKey() 
                     === this.heap[curIdx].getKey() ) {
                if ( this.heap[minChildIdx].getSecondary() 
    ...
    private percUp(curIdx: number): void {
        while (this.hasParent(curIdx)) {
            ...
            if (this.heap[curIdx].getKey() 
                < this.heap[parentIdx].getKey()) {
                this.swap(curIdx, parentIdx);
            } 
            else if ( this.heap[curIdx].getKey() 
                     === this.heap[parentIdx].getKey() ) {
                if ( this.heap[curIdx].getSecondary() 
    ...
    push(item: T, priority?: number, secondary?: number | undefined): void {
        ...
        if (item instanceof Node) {
            this.heap.push(new PriorityNode<T>(item as unknown as T, item.f, item.h));
        }
        else {
            if (!priority) throw new Error("PriorityQueue.push: Priority must be specified.");
