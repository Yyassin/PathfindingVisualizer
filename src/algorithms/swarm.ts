...
const swarm = (startNode: Node, endNode: Node, heuristic: heuristicFunction = manhattanHeuristic) => {
...
const manhattanHeuristic = (nodeA: Node, nodeB: Node): number => {
    return Math.pow(Math.abs(nodeA.x - nodeB.x) + Math.abs(nodeA.y - nodeB.y), 8);
}
